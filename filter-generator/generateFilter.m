...
Nom_Fich2='exp_39500.h';
Nom_Fich3='exp_40000.h';
Nom_Fich4='exp_40500.h';
Nom_Fich5='exp_41000.h';

%% Génération des quatres filtres RIF
filtrecos = zeros(256,5);
filtresin = zeros(256,5);
...
for freq = freqSignalAFiltrerInit:incrementFreq:freqMaxSignal
	i = i +1;
	for k = 1:resolution
			filtrecos(k,i)=floor(resolutionCosinus*cos(2*pi*freq*k/freqEchantillonage));
            filtresin(k,i)=floor(resolutionCosinus*sin(2*pi*freq*k/freqEchantillonage));
	end
end
...
% Génération du fichier cos_39000
fileID = fopen([Nom_Fich1], 'w');
fprintf(fileID,'\n');
fprintf(fileID,'// Exponential filter for the frequency 39,0 kHz\n');
fprintf(fileID,'\n');
fprintf(fileID,'#ifndef _EXP390_H__\n');
fprintf(fileID,'#define _EXP390_H__\n');
fprintf(fileID,'\n');
fprintf(fileID,'\n');
fprintf(fileID,'#include "global.h"\n');
fprintf(fileID,'\n');
fprintf(fileID,'const int32_t cos_39000[%d]= {\n',resolution); 
for  k = 1: resolution
	 fprintf(fileID,'%d,\n',filtrecos(k,1)); % coeff décalage des valeurs pour le code STM (define dans FIR_Filter.h)
end
fprintf(fileID,'};\n'); 
fprintf(fileID,'const int32_t sin_39000[%d]= {\n',resolution); 
for  k = 1: resolution
...
fprintf(fileID,'\n');
fprintf(fileID,'#ifndef _EXP395_H__\n');
fprintf(fileID,'#define _EXP395_H__\n');
fprintf(fileID,'\n');
fprintf(fileID,'\n');
fprintf(fileID,'#include "global.h"\n');
fprintf(fileID,'\n');
...
fprintf(fileID,'\n');
fprintf(fileID,'#ifndef _EXP400_H__\n');
fprintf(fileID,'#define _EXP400_H__\n');
fprintf(fileID,'\n');
fprintf(fileID,'\n');
fprintf(fileID,'#include "global.h"\n');
fprintf(fileID,'\n');
...
fprintf(fileID,'\n');
fprintf(fileID,'#ifndef _EXP405_H__\n');
fprintf(fileID,'#define _EXP405_H__\n');
fprintf(fileID,'\n');
fprintf(fileID,'\n');
fprintf(fileID,'#include "global.h"\n');
fprintf(fileID,'\n');
...
fileID = fopen([Nom_Fich5], 'w');
fprintf(fileID,'\n');
fprintf(fileID,'// Exponential filter for the frequency 39,5 kHz\n');
fprintf(fileID,'\n');
fprintf(fileID,'#ifndef _EXP410_H__\n');
fprintf(fileID,'#define _EXP410_H__\n');
fprintf(fileID,'\n');
fprintf(fileID,'\n');
fprintf(fileID,'#include "global.h"\n');
fprintf(fileID,'\n');
...
for  k = 1: resolution
	 fprintf(fileID,'%d,\n',filtrecos(k,5)); % coeff décalage des valeurs pour le code STM (define dans FIR_Filter.h)
end
fprintf(fileID,'};\n'); 
fprintf(fileID,'const int32_t sin_41000[%d]= {\n',resolution); 
for  k = 1: resolution
	 fprintf(fileID,'%d,\n',filtresin(k,5)); % coeff décalage des valeurs pour le code STM (define dans FIR_Filter.h)
end
fprintf(fileID,'};\n'); 
fprintf(fileID,'\n');
fprintf(fileID,'#endif');
